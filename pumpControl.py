...
class Chain(serial.Serial):
    ...
    def __init__(self, port, baudrate=115200):
        ...
        :type port: str
        """
        serial.Serial.__init__(self, port=port, stopbits=serial.STOPBITS_TWO, parity=serial.PARITY_NONE,
                               bytesize=serial.EIGHTBITS, xonxoff=False, baudrate=baudrate, timeout=2)
        self.flushOutput()
        self.flushInput()
...
class Pump:
    ...
    def infuseDuration(self, durationSec):
        """Start infusing pump."""
        time1 = time.time()
        if self.state == 'idle':
            self.write('irun')
            ...
            print(f"Total time: {time_stop - time1}s")
            print(f"Time after sleep: {time_sleep - time1}s")
            print(f"Time after stop: {time_stop - time1}s")
    ...
    def withdrawDuration(self, durationSec):
        """Start infusing pump."""
        time1 = time.time()
        if self.state == 'idle':
            self.write('wrun')
    ...
    def settargetvolume(self, targetvolume, unit):
        ...
        if self.state == 'idle':
            ...
            if True:
                ...
                returned_targetvolume = convert_units(float(returned_targetvolume), returned_unit + "/min",
                                                      convert_str_units(unit + "/min"))
    ...
    def setsyringevolume(self, vol, unit):
        ...
        if self.state == 'idle':
            ...
            if (resp[-1] == ':' or resp[-1] == '<' or resp[-1] == '>'):
                ...
                returned_volume = convert_units(float(returned_volume), returned_unit + "/min",
                                                convert_str_units(unit + "/min"))
